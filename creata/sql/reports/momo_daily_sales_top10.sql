...
SELECT TOP 10
    ...
    PR.ProdID AS product_id,
    COALESCE(NULLIF(LTRIM(RTRIM(PT.ProdName)), N''), PR.ProdID) AS product_name,
...
JOIN {database}.dbo.comProdRec PR
   ...
   AND BA.Flag = PR.Flag
LEFT JOIN {database}.dbo.comProduct PT
    ON PT.ProdID = PR.ProdID
...
GROUP BY CAST(PR.BillDate AS int),
         PR.ProdID,
         COALESCE(NULLIF(LTRIM(RTRIM(PT.ProdName)), N''), PR.ProdID)
